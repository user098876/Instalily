...
class ScoringService:
    ...
    ENTERPRISE_SIGNAL_KEYWORDS = {"fortune", "locations", "facilities", "plants", "network", "installed base"}
    ...
    def score_company(self, company: Company) -> ScoringRun:
        ...
        if company.employee_count_range:
            scale_points += 4
            scale_reasons.append(f"employee_count_proxy={company.employee_count_range}")
        if company.revenue_estimate:
            scale_points += 4
            scale_reasons.append(f"revenue_proxy={company.revenue_estimate}")
        scale_keyword_points, scale_hits = self._keyword_density_score(
            company_text,
            self.SCALE_KEYWORDS | self.ENTERPRISE_SIGNAL_KEYWORDS,
            max_points=3,
        )
        ...
        if not company.revenue_estimate and not company.employee_count_range:
            caveats.append("Scale is inferred from public proxies only; no validated revenue or employee band stored.")
        ...
        explanations = [
            f"ICP industry fit: {industry_points}/{self.weights.industry_fit} from graphics/signage hits {industry_hits or ['none']}.",
            f"Use-case fit: {application_points}/{self.weights.application_fit} from application hits {application_hits or ['none']}.",
            f"Durability adjacency: {durability_points}/{self.weights.durability_adjacency} from surface-protection hits {durability_hits or ['none']}.",
            f"Ecosystem relevance: {ecosystem_points}/{self.weights.ecosystem_participation} from {len(links)} event/association links.",
            f"Scale signal: {scale_points}/{self.weights.scale_signal} from public/provider proxies {scale_reasons or ['none']}.",
            f"Stakeholder quality: {stakeholder_points}/{self.weights.stakeholder_quality} from stakeholder confidence {stakeholder_reasons or ['none']}.",
            f"Evidence strength: {evidence_confidence_points}/{self.weights.evidence_confidence} from evidence depth/diversity.",
