    ...
    def enrich_company(self, company: Company) -> list[ProviderResult]:
        ...
        for provider in self.providers:
            ...
            if result.status == CrawlStatus.success and result.payload:
                company.description = company.description or result.payload.get("description") or result.payload.get(
                    "description_excerpt"
                )
                company.industry = company.industry or result.payload.get("industry")
                company.employee_count_range = company.employee_count_range or str(
                    result.payload.get("employees") or result.payload.get("employee_count", "")
                )
                company.revenue_estimate = company.revenue_estimate or str(result.payload.get("annual_revenue", ""))
