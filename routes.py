...
def _latest_by_company(rows: list, company_attr: str) -> dict[int, object]:
    latest: dict[int, object] = {}
    for row in rows:
        company_id = getattr(row, company_attr)
...
def list_records(db: Session = Depends(get_db)) -> list[RecordOut]:
    ...
    scores = _latest_by_company(
        db.query(ScoringRun)
        .filter(ScoringRun.company_id.in_(company_ids))
        .order_by(ScoringRun.company_id.asc(), ScoringRun.created_at.desc())
        .all(),
        "company_id",
    )
    stakeholders_by_company = _latest_by_company(
        db.query(Stakeholder)
        .filter(Stakeholder.company_id.in_(company_ids))
        .order_by(Stakeholder.company_id.asc(), Stakeholder.confidence_score.desc())
        .all(),
        "company_id",
    )
    links = _latest_by_company(
        db.query(CompanyEventLink)
        .filter(CompanyEventLink.company_id.in_(company_ids))
        .order_by(CompanyEventLink.company_id.asc(), CompanyEventLink.id.asc())
        .all(),
        "company_id",
    )
    reviews = {
        row.entity_id: row
        for row in db.query(ReviewStatus)
        .filter(ReviewStatus.entity_type == "company", ReviewStatus.entity_id.in_(company_ids))
        .all()
    }
    evidence_by_company: dict[int, list[EvidenceItem]] = {}
    for item in (
        db.query(EvidenceItem)
        .filter(EvidenceItem.entity_type == "company", EvidenceItem.entity_id.in_(company_ids))
        .order_by(EvidenceItem.entity_id.asc(), EvidenceItem.created_at.desc())
        .all()
    ):
        evidence_by_company.setdefault(item.entity_id, [])
        if len(evidence_by_company[item.entity_id]) < 5:
            evidence_by_company[item.entity_id].append(item)

    stakeholder_ids = [row.id for row in stakeholders_by_company.values()]
    drafts_by_stakeholder: dict[int, OutreachDraft] = {}
    if stakeholder_ids:
        for draft in (
            db.query(OutreachDraft)
            .filter(OutreachDraft.stakeholder_id.in_(stakeholder_ids))
            .order_by(OutreachDraft.stakeholder_id.asc(), OutreachDraft.created_at.desc())
            .all()
        ):
            drafts_by_stakeholder.setdefault(draft.stakeholder_id, draft)

    events = {row.id: row for row in db.query(Event).all()}
    associations = {row.id: row for row in db.query(Association).all()}

    for company in companies:
        score = scores.get(company.id)
        stakeholder = stakeholders_by_company.get(company.id)
        link = links.get(company.id)
        draft = drafts_by_stakeholder.get(stakeholder.id) if stakeholder else None
        review = reviews.get(company.id)
        ...
        if link and link.event_id:
            ...
            association = associations.get(link.association_id)
        ...
        if link and _is_valid_http_url(link.source_url):
            evidence_links.append(
                EvidenceLinkOut(
                    ...
                    extraction_method="company_parent_link",
                )
            )
        ...
        if not company.website:
            evidence_caveats.append("Company website was not validated from public evidence or a provider response.")
        if not stakeholder:
            evidence_caveats.append("No qualified stakeholder validated from public pages or optional providers.")
        if not draft:
            evidence_caveats.append("Outreach omitted because evidence-backed facts were insufficient.")
        elif draft.llm_model and "low_confidence" in draft.llm_model:
            evidence_caveats.append("Outreach is low-confidence because the stored fact bundle is thin.")
        ...
        rows.append(
            RecordOut(
                ...
                company_website_status="validated_public_or_provider" if company.website else "missing_unvalidated",
                ...
                qualification_rationale=rationale[0] if rationale else None,
                stakeholder=stakeholder.full_name if stakeholder else None,
                stakeholder_title=stakeholder.title if stakeholder else None,
                stakeholder_profile_url=(stakeholder.profile_url if stakeholder else None),
                ...
                outreach_status=(
                    ...
                    if draft.llm_model and "low_confidence" in draft.llm_model
                    else "ready"
                ),
                outreach_preview=draft.email_opener if draft else None,
                outreach_email_opener=(draft.email_opener if draft else None),
                outreach_linkedin_note=(draft.linkedin_note if draft and draft.linkedin_note else None),
                outreach_three_sentence=(draft.outreach_three_sentence if draft and draft.outreach_three_sentence else None),
...
def export_csv(db: Session = Depends(get_db)) -> Response:
    ...
    for r in records:
        lines.append(
            f'"{_csv_escape(r.event_or_association)}","{_csv_escape(r.company)}","{_csv_escape(r.company_website)}","{_csv_escape(r.qualification_score)}","{_csv_escape(r.stakeholder)}","{_csv_escape(r.stakeholder_title)}","{_csv_escape(r.outreach_status)}","{_csv_escape(r.status)}"'
        )
