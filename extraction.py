...
class ExtractionService:
    ...
    def _process_candidates(
        ...
        for cand in candidates:
            ...
            self._upsert_company_evidence(
                ...
                method=f"structured_roster_parser::{cand.signal}",
            )
    ...
    def _extract_for_parent(
        ...
        summary = {"inserted_companies": 0, "linked_records": 0, "source_pages_scanned": 0, "candidate_pages_matched": 0}

        for url in self._candidate_roster_urls(parent_url):
            summary["source_pages_scanned"] += 1
            ...
            if not candidates:
                continue
            summary["candidate_pages_matched"] += 1
    ...
    def extract_companies(self) -> dict:
        ...
        for event in self.db.query(Event).all():
            ...
            summary["source_pages_scanned"] += evt_summary["source_pages_scanned"]
            summary["candidate_pages_matched"] += evt_summary["candidate_pages_matched"]
        ...
        for association in self.db.query(Association).all():
            ...
            summary["source_pages_scanned"] += assoc_summary["source_pages_scanned"]
            summary["candidate_pages_matched"] += assoc_summary["candidate_pages_matched"]
