...
def discover_likely_roster_links(html: str, base_url: str, max_links: int = 8) -> list[str]:
    ...
    for anchor in soup.select("a[href]"):
        ...
        anchor_text = clean_text(anchor.get_text(" ", strip=True)).lower()
        url_text = f"{parsed.path} {parsed.query} {anchor_text}"
        ...
        if any(term in parsed.path.lower() for term in ["exhibitor", "member", "directory", "roster", "sponsor"]):
            score += 6
        if any(term in anchor_text for term in ["official", "full list", "all exhibitors", "member directory"]):
            score += 4
